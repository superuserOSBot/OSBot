...
public class MindRunes {

	public static Area ruinsArea = new Area(new Position(2977, 3509, 0), new Position(2987, 3519, 0));
	public static Position ruinsPosition = new Position(2984, 3516, 0);

	public static Area altarArea = new Area(new Position(2762, 4819, 0), new Position(2802, 4854, 0));
	public static Position altarPosition = new Position(2788, 4839, 0);

	public static Area bankArea = new Area(new Position(2943, 3366, 0), new Position(2949, 3373, 0));
	public static Position bankPosition = new Position(2946, 3368, 0);
	...
	public static State getState(Script script) throws InterruptedException {
		if (!script.getInventory().isFull()) {
			if (!script.getEquipment().isWearingItem(EquipmentSlot.HAT, "Mind tiara")) {
				if (!script.getInventory().contains("Mind tiara")) {
			...
			if (bankArea.contains(script.myPlayer())) {
				if (script.getBank().isOpen()) {
					if (script.getInventory().contains("Mind rune")) {
	...
	public static void doTask(Script script) throws InterruptedException {
		...
		case WITHDRAW_TIARA:
			...
			Bank.depositAllExcept(script, "Mind tiara");
			Bank.withdraw(script, 1, "Mind tiara");
			break;
		...
		case EQUIP_TIARA:
			...
			MyPlayer.equip(script, EquipmentSlot.HAT, "Mind tiara");
			break;
		...
		case CRAFT_RUNES:
			...
			Objects.interact(script, "Craft-rune", Objects.find(script, "Altar"), "Mind rune");
			break;
